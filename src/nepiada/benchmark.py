    ...
    for iteration in iterations:
        ...
        for noise in noise_types:
            ...
            for radius in observation_radii:
                ...
                if run_dqn:
                    # Run the baseline.py script and save the output
                    subprocess.run(["python", os.path.join(baseline_script_dir, "rl_ma_dqn_experimental.py"), str(seed), "2", "4", "3", "2", str(radius), noise, str(iteration)], stdout=open(f"dqn_logs_{seed}_{iteration}_{noise}_{radius}.txt", "w"))
